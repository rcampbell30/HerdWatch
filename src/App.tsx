...
const riskCopy: Record<RiskStatus, { label: string; description: string; className: string }> = {
  AT_RISK: {
    label: 'AT RISK',
    description: 'Below 90% — outbreak vulnerability is high.',
    className: 'risk'
  },
  VULNERABLE: {
    label: 'VULNERABLE',
    description: '90–95% — below the herd-immunity target.',
    className: 'vulnerable'
  },
  PROTECTED: {
    label: 'PROTECTED',
    description: 'At or above the 95% herd-immunity target.',
    className: 'protected'
  }
};
      ...
      <div className="chart-wrap" aria-label="MMR vaccination coverage chart">
        <ResponsiveContainer width="100%" height={320}>
          <LineChart data={data} margin={{ top: 10, right: 18, left: 0, bottom: 10 }}>
            ...
            <YAxis domain={[75, 100]} tickFormatter={(value) => `${value}%`} tick={{ fontSize: 12 }} />
            <Tooltip formatter={(value: number) => `${Number(value).toFixed(1)}%`} />
            <ReferenceLine y={target} label="95% target" strokeDasharray="5 5" />
...
function Hero({ onSearch }: { onSearch: (value: string) => void }) {
  ...
  const results = useMemo(() => {
    ...
    if (!trimmed) return [];
    return areas
      .filter((area) => area.postcodeDistrict.includes(trimmed) || area.region.toUpperCase().includes(trimmed))
      .slice(0, 8);
  }, [query]);
      ...
      <div className="hero-inner">
        ...
        <div className="search-wrap">
          <input
            ...
            placeholder="Search postcode district — FY1, M15, LS12..."
            value={query}
...
function AreaCard({ area }: { area: HerdArea }) {
  ...
  return (
    <a className="town-card" href={`/town/${area.postcodeDistrict.toLowerCase()}/`}>
      <div>
        <div className="town-district">{area.postcodeDistrict}</div>
        <div className="town-meta">{area.practiceCount} practices · {area.region}</div>
      </div>
      <div className="town-card-right">
        <div className={`town-cov ${meta.className}-text`}>{formatPercent(area.coverage)}</div>
        <div className={`town-badge ${meta.className}`}>{meta.label}</div>
      </div>
    </a>
  );
}
    ...
    <>
      ...
      <main className="main-content">
        ...
        <div className="cta-row">
          <a className="btn btn-red" href="/map/">View Map →</a>
          <a className="btn btn-dark" href="/towns/">All Areas →</a>
          <a className="btn btn-outline" href="/methodology/">Methodology</a>
        </div>
      </main>
    </>
  );
  ...
  return (
    <main className="main-content page-shell">
      ...
      <div className="table-card">
        <table>
          <thead>
            <tr><th>Area</th><th>Region</th><th>Practices</th><th>Coverage</th><th>Status</th><th>Gap to 95%</th></tr>
          </thead>
          <tbody>
            {filtered.map((area) => (
              <tr key={area.postcodeDistrict}>
                ...
                <td>{formatPercent(area.coverage)}</td>
...
function TownPage({ area }: { area?: HerdArea }) {
  ...
  const nearby = areas
    .filter((item) => item.region === area.region && item.postcodeDistrict !== area.postcodeDistrict)
    .sort((a, b) => Math.abs(a.coverage - area.coverage) - Math.abs(b.coverage - area.coverage))
    .slice(0, 5);
  ...
  return (
    <>
      <section className="hero-band">
        <div className="hero-inner">
          <div className="breadcrumb"><a className="bc-link" href="/">Home</a><span className="bc-sep">/</span><a className="bc-link" href="/towns/">Areas</a><span className="bc-sep">/</span><span className="bc-cur">{area.postcodeDistrict}</span></div>
          <h1 className="hero-title"><span className="postcode">{area.postcodeDistrict}</span> vaccination coverage</h1>
        </div>
      </section>
      <main className="content-layout">
        <section className="main-col">
          <div className={`status-banner ${meta.className}`}>
            <div>
              <div className="status-threat">Threat level</div>
              <div className="status-label">{meta.label}</div>
              <div className="status-desc">{meta.description}</div>
            </div>
            <div className="status-right">
              <div className="big-coverage">{formatPercent(area.coverage)}</div>
              <div className="cov-label">MMR1 coverage at 24 months</div>
            </div>
          </div>

          <div className="metric-grid card">
            <Metric label="Eligible children" value={area.totalEligible.toLocaleString()} />
            <Metric label="Vaccinated" value={area.totalVaccinated.toLocaleString()} />
            <Metric label="Unvaccinated" value={unvaccinated(area).toLocaleString()} />
        ...
        <aside className="side-col">
          ...
          <div className="side-card">
            <h2 className="side-title">Nearby / similar areas</h2>
            {nearby.map((item) => <AreaCard key={item.postcodeDistrict} area={item} />)}
          </div>
          <div className="gp-cta">
            <h3>Need vaccination guidance?</h3>
            <p>Use official NHS advice or contact your GP practice.</p>
            <a className="gp-btn" href="https://www.nhs.uk/vaccinations/mmr-vaccine/" target="_blank" rel="noreferrer">NHS MMR advice</a>
          </div>
        </aside>
      </main>
    </>
      ...
      <div className="map-placeholder">
        {areas.slice(0, 18).map((area) => (
          <a key={area.postcodeDistrict} className={`map-cell ${riskCopy[area.status].className}`} href={`/town/${area.postcodeDistrict.toLowerCase()}/`}>
            <strong>{area.postcodeDistrict}</strong>
            <span>{formatPercent(area.coverage)}</span>
          </a>
        ))}
      </div>
    </main>
...
function MythsPage() {
  return (
    <main className="main-content page-shell readable">
      <PageTitle eyebrow="The myth" title="Herd immunity is not magic" description="It is a threshold effect: when enough people are immune, infections have fewer routes through a community." />
      <section className="card prose-card">
        <h2>Why the 95% line matters</h2>
        <p>Measles is unusually infectious. Small drops in vaccination coverage can therefore create large changes in local outbreak vulnerability.</p>
        <p>HerdWatch should avoid shaming individual families. The stronger civic angle is: low coverage reveals a system-level vulnerability, especially for babies, immunocompromised people and anyone who cannot be vaccinated.</p>
      </section>
    </main>
  );
}
